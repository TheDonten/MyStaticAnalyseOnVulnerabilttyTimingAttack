...
void Find_Path(Function *f, std::vector<Node_inst*> nodes) {
    std::string flow_path;
    StringRef check;
    std::set<std::string> out;
    Node_inst* first = *Secret_variable.begin();
        ...
        for (auto inst = block->begin(); inst != block->end(); inst++) {
            ...
            if (isa<UnaryOperator>(inst))
                ...
                for (int i = 0; i < inst->getNumOperands(); i++) {
                    ...
                    if (Instruction* k = dyn_cast<Instruction>(inst->getOperand(i))) {
                        
                            if (isa<StoreInst>(inst) && i == 0) {
                                std::string name1 = k->getName().str(), name2 = inst->getOperand(i + 1)->getName().str();
                                if (!inst->getOperand(0)->hasName()) {
                                    name1 = find_original_var(cast<Instruction>(inst->getOperand(0)));
                                }
                                if (!inst->getOperand(1)->hasName()) {
                                    name2 = find_original_var(cast<Instruction>(inst->getOperand(1)));
                                }
                               
                                if (name2 != "" && name1 != "") {
                                    auto n1 = std::find_if(nodes.begin(), nodes.end(), [=](Node_inst* e) {return e->get_value() == inst->getOperand(0); });
                                    if(n1 != nodes.end()){
                                        Node_inst* prom = *n1;
                                        //std::cout << first->get_object_name() << std::endl;
                                        if (prom->is_secret() || prom->secret_index) {
                                            //find_repeat(&out, '\"' + name1 + '\"' + " -> " + '\"' + name2 + '\"' + ";\n");
                                            out.insert('\"' + name1 + '\"' + " -> " + '\"' + name2 + '\"' + "[color = blue];\n");
                                        }
                                        else
                                            out.insert('\"' + name1 + '\"' + " -> " + '\"' + name2 + '\"' + ";\n");
                                    }
                                }

                                break;
                            }
                            else if (isa<BinaryOperator>(inst) && i == 0) {
                       
                                std::string name_inst = inst->getName().str();
                                ...
                                if (name2 != "") {
                                    //out.insert('\"' + name2 + '\"' + " -> " + '\"' + name_inst + '\"' + ";\n");
                                    auto n1 = std::find_if(nodes.begin(), nodes.end(), [=](Node_inst* e) {return e->get_value() == inst->getOperand(1); });
                                    if (n1 != nodes.end()) {
                                        
                                        Node_inst* prom = *n1;

                                        if (prom->is_secret() || prom->secret_index) {
                                            //find_repeat(&out, '\"' + name2 + '\"' + " -> " + '\"' + name_inst + '\"' + ";\n");
                                            out.insert('\"' + name2 + '\"' + " -> " + '\"' + name_inst + '\"' + "[color = blue];\n");//red label
                                        }
                                        else
                                            out.insert('\"' + name2 + '\"' + " -> " + '\"' + name_inst + '\"' + ";\n");
                                    }
                                ...
                                if (name1 != "") {
                                    ...
                                    if (n1 != nodes.end()) {
                                        Node_inst* prom = *n1;
                                        if (prom->is_secret() || prom->secret_index) {
                                           // find_repeat(&out,'\"' + name1 + '\"' + " -> " + '\"' + name_inst + '\"' + ";\n");
                                            out.insert('\"' + name1 + '\"' + " -> " + '\"' + name_inst + '\"' + "[color = blue];\n");//red label
                                        }
                                        else
                                            out.insert('\"' + name1 + '\"' + " -> " + '\"' + name_inst + '\"' + ";\n");
                                    }
                                   // out.insert('\"' + name1 + '\"' + " -> " + '\"' + name_inst + '\"' + ";\n");
                                }
                            ...
                            else if (inst->hasName() && k->hasName()) {
                                Value* val1 = cast<Value>(inst), *val2 = cast<Value>(k);
                                auto n1 = std::find_if(nodes.begin(), nodes.end(), [=](Node_inst* e) {return e->get_value() == val1; });
                                auto n2 = std::find_if(nodes.begin(), nodes.end(), [=](Node_inst* e) {return e->get_value() == val2; });
                                if (n1 != nodes.end() && n2 != nodes.end()) {
                                    Node_inst* prom1 = *n1, * prom2 = *n2;

                                    if ((prom2->is_secret() || prom2->secret_index)) {
                                        //find_repeat(&out, '\"' + k->getName().str() + '\"' + " -> " + '\"' + inst->getName().str() + '\"' + ";\n");
                                        out.insert('\"' + k->getName().str() + '\"' + " -> " + '\"' + inst->getName().str() + '\"' + "[color = blue];\n");
                                    }
                                    else
                                        out.insert('\"' + k->getName().str() + '\"' + " -> " + '\"' + inst->getName().str() + '\"' + ";\n");
                                    out.insert('\"' + k->getName().str() + '\"' + " [ color = red]\n");
                                }
                    ...
                    if (!inst->getOperand(i)->hasName()) {
                        name = find_original_var(cast<Instruction>(inst->getOperand(i)));
                    }
                    else {
                        name = inst->getOperand(i)->getName().str();
                    }

                    if (name != "" && val->hasName()) {

                        auto n1 = std::find_if(nodes.begin(), nodes.end(), [=](Node_inst* e) {return e->get_value() == inst->getOperand(i); });
                        if (n1 != nodes.end()) {
                            Node_inst* prom = *n1;
                            if(prom->is_secret() || prom->secret_index){
                                //find_repeat(&out, '\"' + name + '\"' + " -> " + '\"' + val->getName().str() + '\"' + ";\n");
                                out.insert('\"' + name + '\"' + " -> " + '\"' + val->getName().str() + '\"' + "[color = blue];\n");
                            }
                            else {
                                out.insert('\"' + name + '\"' + " -> " + '\"' + val->getName().str() + '\"' + ";\n");
                            }
                            out.insert('\"' + name + '\"' + " [ color = red]\n");
